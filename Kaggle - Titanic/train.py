...
imp_custom_num = SimpleImputer(
    missing_values=np.nan, strategy="mean", add_indicator=False, copy=False
)
...
imp_custom_cat = SimpleImputer(
    missing_values=np.nan, strategy="most_frequent", add_indicator=False, copy=False
)
...
pd.merge(
    ...
    sql,
    how="left",
    left_on=["Sex_female", "C_type"],
    right_on=["Index 0", "Index 1"],
)
...
for learning_rate in lr_list:
    for n_estimators in ne_list:
        ...
        print(
            ...
            "N=",
            n_estimators,
            "TR={0:.3f}".format(gb_clf.score(x_train, y_train)),
            "VAL={0:.3f}".format(gb_clf.score(x_val, y_val)),
            "OF={0:.3f}".format(ofit),
        )
...
for learning_rate in lr_list:
    for n_estimators in ne_list:
        ...
        xgb_clf2 = XGBClassifier(
            n_estimators=n_estimators, learning_rate=learning_rate, random_state=10
        )
        ...
        b_row = pd.DataFrame(
            {
                "acc": [xgb_clf2.score(x_val, y_val)],
                "nest": [n_estimators],
                "l_rate": [learning_rate],
                "ofit": [ofit],
            }
        ...
        print(
            ...
            "N=",
            n_estimators,
            "TR={0:.3f}".format(xgb_clf2.score(x_train, y_train)),
            "VAL={0:.3f}".format(score),
            "OF={0:.3f}".format(ofit),
        )
